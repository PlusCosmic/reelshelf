...
- The authoritative check and the insert run under a per-owner Postgres advisory lock (`pg_advisory_xact_lock`), so concurrent uploads cannot pass the check on the same stale usage figure. The clip row is reserved with a placeholder video id before anything is created at Bunny (collection included), and the Bunny video is attached to it afterwards, so a rejected request never spends a Bunny operation and no network I/O happens while a pool connection holds the lock. If Bunny creation fails the reservation is deleted again; if a created video cannot be attached and also cannot be deleted, its real id is recorded on the row so the abandoned-upload purge removes it later. Gaming sessions deduplicate participant ids and allow at most 12 people.
...
- Only a clip's owner can add it to a playlist. Playlist membership itself grants read access, so letting a collaborator copy someone else's clip into another playlist would create a grant that outlives the owner revoking the original share or collaboration. Gaming-session participants are limited to people who have personally added the caller to a collection they created; rows added by other collaborators do not count, since any collaborator can add users and that would let a third party manufacture the creator's consent.
- FFmpeg downloads are capped process-wide (`FFmpeg:MaxConcurrentDownloads`, default 3, 503 after a 30 s queue wait), run one at a time per video, and write unique files per request. A permit is held until the response has been streamed, so slow clients cannot accumulate finished files beyond the cap.
- Endpoints that spend a shared budget are rate limited per user: game search and add-from-IGDB (`Igdb:RequestsPerUserPerMinute`, default 30) and clip preparation (`Uploads:PreparationsPerUserPerMinute`, default 60, well above what the three-at-a-time uploader can reach). IGDB calls are additionally paced process-wide to IGDB's 4 requests per second, cached, and capped in concurrency, since they use one shared set of application credentials.
...
- A clip whose upload is abandoned still holds its reserved storage. The uploader deletes prepared clips on cancel, error, and retry, and the status refresh service purges clips older than 24 hours that were never uploaded or that Bunny marked failed with nothing stored. Deleting a video Bunny no longer has is treated as success so a retried delete can always finish.
...
- Reads by clip id (`GET /api/clips/videos/{id}`, view marking, ffmpeg download) are scoped to the owner and current members of a playlist containing the clip. Share links are honoured only through `GET /api/shared-clips/{token}`, so knowing a clip's UUID never substitutes for holding the token. Previously any signed-in account could read any clip by id.
