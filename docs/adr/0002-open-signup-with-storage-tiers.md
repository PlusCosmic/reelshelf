...
- The authoritative check and the insert run under a per-owner Postgres advisory lock (`pg_advisory_xact_lock`), so concurrent uploads cannot pass the check on the same stale usage figure. The clip row is reserved with a placeholder video id before anything is created at Bunny, and the Bunny video is attached to it afterwards, so a rejected request never spends a Bunny operation and no network I/O happens while a pool connection holds the lock. If Bunny creation fails the reservation is deleted again. Gaming sessions deduplicate participant ids and allow at most 12 people.
