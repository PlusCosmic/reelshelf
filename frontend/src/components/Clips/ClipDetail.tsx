...
export function ClipDetail({ clipId, categorySlug }: ClipDetailProps) {
  ...
  const { data: clipOwner, isLoading: loadingClipOwner } = useUserById(
    clip?.ownerId,
  );
  const markAsViewed = useMarkAsViewed();

  // Custom hooks for business logic
  const { tagsValue, setTagsValue, titleValue, setTitleValue, topTags } =
    useClipFormState(clip);
  const { relatedClips, isLoading: loadingRelatedClips } = useRelatedClips(
    clipId,
    category?.id,
  );
