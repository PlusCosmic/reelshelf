...
import type { PlaylistCollaborator } from "@/api-client";
...
export function PlaylistCollaboratorsModal({
  ...
  const addMutation = useMutation({
    mutationFn: (username: string) => addCollaborator(playlistId, { username }),
    onSuccess: () => {
      notifications.show({
  ...
  const handleRemoveCollaborator = (userId: string) => {
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">Remove this collaborator from the playlist?</Text>
      ),
    ...
    <Modal
      ...
      styles={{
        content: {
          ...
          boxShadow:
            "0 20px 60px rgba(0, 0, 0, 0.5), 0 0 40px rgba(168, 85, 247, 0.1)",
        },
        header: {
      ...
      <Stack gap="lg">
        ...
        <Stack gap="xs">
          ...
          <Text size="xs" c="dimmed">
            Collaborators can add clips to this playlist. They'll receive a
            Discord DM notification if enabled.
          </Text>
        </Stack>
