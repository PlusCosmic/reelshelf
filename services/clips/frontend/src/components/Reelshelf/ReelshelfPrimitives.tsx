...
export function Avatar({ name = "You", src, size = 28 }: { name?: string | null; src?: string | null; size?: number }) {
  ...
  const initials = (name ?? "You")
    ...
    .join("");
  return (
    <span className="rs-avatar" style={{ "--size": `${size}px` } as CSSProperties}>
      {src && !imageFailed ? (
        <img src={src} alt={name ?? "User avatar"} onError={() => setImageFailed(true)} />
      ) : (
        initials || "Y"
      )}
    </span>
...
export function Chip({
  ...
  return (
    <button className={`rs-chip${active ? " active" : ""}`} type="button" onClick={onClick}>
      {children}
    </button>
  );
}
...
export function GameVars({ category, children }: { category?: GameCategoryResponse | null; children: React.ReactNode }) {
...
export function ClipThumb({
  ...
  return (
    ...
    >
      ...
      {!compact && firstTag ? <span className="rs-tag-badge">{firstTag}</span> : null}
      {clip.video.length > 0 ? <span className="rs-duration">{formatDuration(clip.video.length)}</span> : null}
    </button>
  );
}
...
export function ClipCard({
  ...
  return (
    <article className="rs-card">
      <Link to="/games/$slug/$clipId" params={{ slug: clip.categorySlug, clipId: clip.clipId }}>
...
export function ClipGrid({
  ...
  if (clips.length === 0) return <div className="rs-empty">No clips match this view.</div>;
      ...
      <div className="rs-filmstrip">
        {clips.map((clip, index) => {
          ...
          return (
            ...
            >
              <span className="rs-display" style={{ color: "var(--fg-soft)", fontSize: 25 }}>
                {String(index + 1).padStart(2, "0")}
              </span>
              <ClipThumb clip={clip} category={category} compact />
              <span>
                <strong style={{ display: "block", fontSize: 14 }}>{clip.video.title}</strong>
                <span className="rs-meta">{clip.tags.slice(0, 3).map((tag) => `#${tag}`).join(" ")}</span>
              </span>
              <span className="wide-only rs-meta">{category?.name ?? "Game"}</span>
              <span className="wide-only rs-meta">{formatDuration(clip.video.length)}</span>
            </Link>
          );
        })}
      </div>
  ...
  return (
    <div className={variant === "grid" ? "rs-compact-grid" : "rs-grid"}>
      {clips.map((clip) => (
        <ClipCard key={clip.clipId} clip={clip} categories={categories} compact={variant === "grid"} />
      ))}
    </div>
  );
}
...
export function StatLine({ clips }: { clips: Clip[] }) {
  const totalSeconds = clips.reduce((sum, clip) => sum + (clip.video.length || 0), 0);
  const totalBytes = clips.reduce((sum, clip) => sum + (clip.video.storageSize || 0), 0);
  return (
    <span>
      {clips.length} clips - {formatDuration(totalSeconds)} - {formatSize(totalBytes)}
    </span>
  );
}
