...
export function useClipsFilters() {
  ...
  const [todayFilterActive, setTodayFilterActive] = useAtom(todayFilterActiveAtom);
  ...
  const hasActiveFilters = searchQuery.length > 0 || selectedTags.length > 0 || showUnviewed || sortOrder !== undefined || startDate !== undefined || endDate !== undefined;
