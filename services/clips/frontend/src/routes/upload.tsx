...
type UploadStatus = "idle" | "ready" | "hashing" | "creating" | "uploading" | "paused" | "done" | "error";
...
function UploadRoute() {
  ...
  const step = status === "idle" || status === "ready" || (status === "error" && !file) ? 1 : status === "done" || status === "error" ? 3 : 2;
  const canUpload = Boolean(file && selectedCategoryId && title.trim()) && (status === "ready" || status === "error");
  ...
  async function startUpload() {
    ...
    try {
      ...
      const response = await createVideo.mutateAsync({
        ...
        title: title.trim(),
        md5Hash,
        createdAt: createdAt ? new Date(createdAt) : new Date(file.lastModified),
      });
  ...
  return (
    <section className="rs-upload-page">
      ...
      <input ref={fileInputRef} className="rs-visually-hidden" type="file" accept="video/mp4,video/quicktime,video/webm,video/x-matroska" onChange={onInputChange} />
      ...
      {step === 1 ? (
        ...
        >
          ...
          <span className="rs-upload-drop-title">Drop a video, or click to browse</span>
          <span className="rs-upload-drop-copy">MP4, MOV, WebM, or MKV up to 4 GB.</span>
        </button>
      ) : null}
        ...
        <div className="rs-upload-panel">
          ...
          <div className="rs-upload-form-grid">
            <div className="rs-field">
              ...
              <input id="clip-title" value={title} onChange={(event) => setTitle(event.currentTarget.value)} disabled={status === "hashing" || status === "creating" || status === "uploading"} />
            </div>
            <div className="rs-field">
              <label htmlFor="clip-category">Game</label>
              <select id="clip-category" value={selectedCategoryId} onChange={(event) => setCategoryId(event.currentTarget.value)} disabled={categoriesLoading || status === "hashing" || status === "creating" || status === "uploading"}>
            ...
            <div className="rs-field">
              ...
              <input id="clip-created" type="datetime-local" value={createdAt} onChange={(event) => setCreatedAt(event.currentTarget.value)} disabled={status === "hashing" || status === "creating" || status === "uploading"} />
            </div>
          </div>
          ...
          <div className="rs-upload-actions">
            {status === "ready" || status === "error" ? (
              <button className="rs-primary" type="button" disabled={!canUpload || createVideo.isPending} onClick={startUpload}>
                <IconUpload size={15} />
                Upload clip
              </button>
            ) : null}
            {status === "uploading" ? (
              <button className="rs-secondary" type="button" onClick={pauseUpload}>
                <IconPlayerPause size={15} />
                Pause
              </button>
            ) : null}
            {status === "paused" ? (
              <button className="rs-primary" type="button" onClick={resumeUpload}>
                <IconPlayerPlay size={15} />
                Resume
              </button>
            ) : null}
            {status === "done" ? (
              <button className="rs-primary" type="button" onClick={() => void navigate({ to: "/" })}>
                <IconCheck size={15} />
                Back to library
              </button>
            ) : null}
            <button className="rs-secondary" type="button" onClick={resetUpload}>
              {status === "done" || status === "error" ? <IconRotateClockwise size={15} /> : <IconX size={15} />}
              {status === "done" || status === "error" ? "Start over" : "Cancel"}
            </button>
          </div>
        </div>
      ) : null}
