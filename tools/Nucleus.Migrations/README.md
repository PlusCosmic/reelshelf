...
This project owns database migrations for the split Nucleus services.
...
The infrastructure repository's `services/clips.yml` compose file runs this project as a one-shot `nucleus-migrations` container and makes the clips API service depend on successful completion. Keep a single migration owner per database.
...
```sh
DatabaseConnectionString="Host=...;Database=...;Username=...;Password=..." dotnet run --project tools/Nucleus.Migrations
```

For first-time production adoption:

```sh
DatabaseConnectionString="Host=...;Database=...;Username=...;Password=..." dotnet run --project tools/Nucleus.Migrations -- --adopt-existing
```
